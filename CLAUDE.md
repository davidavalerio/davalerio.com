...
Static HTML. Source Serif 4 via Google Fonts. The homepage is hand-made with inline CSS; the only JavaScript on the site is `theme.js`, the light/dark switch. Only the writing list inside the homepage is generated. The writing section is generated by `_writing/build.py` (Python, PyYAML, Pandoc) and the generated pages are committed, so GitHub Pages serves plain files.
...
- `theme.js` — the light/dark switch (see Design System)
...
**Dark mode** follows the device setting until the reader uses the small Dark / Light switch (top right of the card screen on the homepage, beside Writing in the masthead on the pieces). The choice is remembered in that browser, and choosing the side the device already uses returns to following the device. `theme.js` (loaded in every page head, before the styles, so there is no flash) holds that logic and sets `data-theme` on the page; the switch is hidden when scripting is off. The dark token values (screen only, so print stays light) sit in both `index.html` and `writing/style.css`, each written twice — once for the device setting, once for the reader's choice — paper `#16191B`, ink `#E4E0D8`, stone `#9BA0A4`, hairline `#32383C`, accent `#CB8B5C` (iron oxide lightened to stay readable), block quotes `#C2C0BA`. Keep every color on a token so both modes stay in step; the two files' values must match.

## Layout

The calling card is centered both vertically and horizontally in a first section that is exactly one screen (`.card-screen`, `100svh`), so the page opens as the card alone — no nav, no footer, and none of the writing list showing; the only other thing on the screen is the light/dark switch in the top right corner. The card's Writing link scrolls to `#writing`, the section below: a 41rem column with the heading, jump links to Essays / Papers / Conversations, and the three grouped lists, newest first (dates drop under titles on phones).
